class Book {
  ...
  get info() {
    return (
      ...
      this.pages +
      " pages, " +
      (this.completed ? "read already" : "not read yet")
    );
  }
}
...
class Library {
  ...
  static DELETE_SVG =
    '<svg xmlns="http://www.w3.org/2000/svg" width="30px" viewBox="0 0 24 24"><title>delete</title><path d="M19,4H15.5L14.5,3H9.5L8.5,4H5V6H19M6,19A2,2 0 0,0 8,21H16A2,2 0 0,0 18,19V7H6V19Z" /></svg>';
  ...
  constructor() {
    ...
    this.newBookForm.addEventListener("submit", (e) => {
      ...
      let newBook = new Book(
        ...
        formDataObject.completed != undefined
      );
  ...
  addBook(book) {
    ...
    bookElement.style.setProperty(
      "--book-color",
      Library.BOOK_COLORS[
        Math.floor(Math.random() * Library.BOOK_COLORS.length)
      ]
    );
    bookElement.style.setProperty(
      "--book-title-font",
      Library.BOOK_FONTS[Math.floor(Math.random() * Library.BOOK_FONTS.length)]
    );
    ...
    completedCheckbox.setAttribute(
      "id",
      `completedID:${this.myLibrary.length}`
    );
    completedCheckbox.onclick = () => {
      this.myLibrary[completedCheckbox.id.split(":")[1]].completed =
        completedCheckbox.checked;
      window.localStorage.setItem("myLibrary", JSON.stringify(this.myLibrary));
    };
    ...
    deleteButton.onclick = () => {
      ...
      this.myLibrary.splice(targetBook.id, 1);
      targetBook.remove();
      window.localStorage.setItem("myLibrary", JSON.stringify(this.myLibrary));
    };
...
if (window.localStorage.getItem("myLibrary")) {
  let booksToAdd = JSON.parse(window.localStorage.getItem("myLibrary"));
  for (book of booksToAdd) {
  ...
  mainLibrary.addBook(
    new Book("Tales of Dunk and Egg", "George R. R. Martin", 860, false)
  );
  mainLibrary.addBook(
